...
export function ListLikeFollowUI({ listId, isLiked, isFollowed, isOwner }: { listId: string, isLiked: boolean, isFollowed: boolean, isOwner: boolean }) {
  ...
  return (
     <div className={styles.socialActions}>
       <button onClick={() => startTransition(() => toggleListLikeAction(listId))} disabled={isPending} className={`${styles.socialBtn} ${isLiked ? styles.liked : ''}`}>
          {isLiked ? <Heart size={16} fill="currentColor" /> : <Heart size={16} />}
          <span>{isLiked ? 'Liked' : 'Like'}</span>
       </button>
       <button onClick={() => startTransition(() => toggleListFollowAction(listId))} disabled={isPending} className={`${styles.socialBtn} ${isFollowed ? styles.followed : ''}`}>
...
export function OwnerRowActions({ listId, gameId, isCurrentCover }: { listId: string, gameId: number, isCurrentCover: boolean }) {
   ...
   return (
      <div className={styles.rowActions}>
         {!isCurrentCover ? (
           <button onClick={() => startTransition(() => setListCoverAction(listId, gameId))} disabled={isPending} className={styles.iconBtn} title="Set as List Cover">
              <ImagePlus size={18} />
           </button>
         ) : (
           <span className={styles.currentCoverIcon} title="Current Cover"><ImageIcon size={18} /></span>
         )}
         <button onClick={() => startTransition(() => removeGameFromListAction(listId, gameId))} disabled={isPending} className={styles.iconBtnRemove} title="Remove from List">
            <X size={18} />
         </button>
      </div>
   )
